  ...
  return (
    <>
      ...
      <div className="flex items-center gap-3">
        ...
        <img
          src="https://i.postimg.cc/j2tHGNC5/image.png"
          alt="category icon"
            ...
            return (
              <Link to={`/product/${product.id}`} key={product.id} className="h-full">
                <div className="flex flex-col h-full gap-3 pb-3 group shadow-md rounded-lg overflow-hidden transition-transform duration-300 hover:shadow-lg bg-[#FFF2EB] relative">
                  ...
                  <div className="px-3 pb-4 flex flex-col justify-between h-[190px]">
                    ...
                    <button className="mt-auto py-2 px-1 rounded-full bg-[#FCBACB] text-white text-sm font-semibold shadow-md hover:bg-gray-900 transition-all duration-200">
                      {activeDiscount ? `Buy Now - PKR ${discountedPrice.toLocaleString()}` : 'Buy Now'}
                    </button>
                  </div>
                </div>
              </Link>
